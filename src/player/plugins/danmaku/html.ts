...
<span class="label--not-pressed plyr__tooltip">弹幕设置</span>
  ...
  $danmaku = popover({
    ...
    onVisibleChange: (visible) => {
      this.player.plyr.elements.container?.classList.toggle(
      ...
      if (visible) {
        ...
        this.player.plyr.elements.container?.prepend(dom)
      } else {
        this.player.plyr.elements.container
          ?.querySelector('.k-player-danmaku-overlay-video-prevent-click')
          ?.remove()
      }
    },
  })
